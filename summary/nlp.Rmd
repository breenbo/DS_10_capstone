...
library(dplyr)
head(twitTest)
dfTwit <- data.table(line=1:2000, text=twitTest)
str(dfTwit)
object.size(dfTwit)
tidyText <- dfTwit %>% unnest_tokens(word, text)
str(tidyText)
head(tidyText)
object.size(tidyText)
```
...
```{r}
twoGrams <- dfTwit %>% unnest_tokens(ngrams, text, token="ngrams", n=2)
str(twoGrams)
head(twoGrams)
```

### Stopwords
Use `anti_join()` function.
```{r}
data(stop_words)
stopText <- tidyText %>% anti_join(stop_words)
dim(stopText)
head(stopText)
object.size(stopText)
```
