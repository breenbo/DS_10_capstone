...
wordCloudPlot <- function(tidyText, nb=100) {
    ...
    wordcloud(words=tidyText$ngram, freq=tidyText$n, max.words=nb, random.order=F, rot.per=0.35, scale=c(5,0.5), colors=brewer.pal(8,"Dark2"))
}
...
coverWords <- function(tidyText, n1=50, n2=90) {
    # take a tidyText and return a data.frame with numbers of unique words to describe n1 and n2 % of the total dataset.
    library(dplyr)
    tidyText <- tidyText %>% count(ngram, sort=TRUE) %>%
        mutate(ngram=reorder(ngram,n)) %>%
        mutate(percent=n/sum(n)*100) %>%
    ...
    if(nbgram==1){
        ...
        # name.x3 <- paste('total unique ',nbgram,'-grams', sep="")
...
Plots of the relationship of the 150 most commons bigrams and trigrams of samples of the 3 datasets.
